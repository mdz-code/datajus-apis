...
class SupabaseModel {
    ...
    async queryBuilder(table, keyQuery, valueQuery, selectQuery=['*']) {
        const stringSelect = selectQuery.join(',')
        const { data, error } = await this.supabase.from(table).select(stringSelect).eq(keyQuery, valueQuery).limit(1)
